...
const Scene3D: React.FC<SceneProps> = ({ gameState }) => {
  ...
  useEffect(() => {
    ...
    const positions = [
      ...
      { x: 1, z: 1.3 },
      { x: -1, z: -1.3 },
      { x: 1, z: -1.3 }, // Fixed: Changed last coordinate x from -1 to 1
    ];
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      if (tireSlip > 0.2 && Math.abs(speed) > 1) {
        ...
        if (Math.random() > 0.6 && smokeParticlesRef.current) {
          ...
          xOffsets.forEach((xOffset) => {
            ...
            const wx =
              x +
              Math.sin(heading + Math.PI / 2) * xOffset +
              Math.sin(heading) * wheelOffsetZ;
            const wz =
              z +
              Math.cos(heading + Math.PI / 2) * xOffset +
              Math.cos(heading) * wheelOffsetZ;
      ...
      if (smokeParticlesRef.current) {
        for (
          let i = smokeParticlesRef.current.children.length - 1;
          i >= 0;
          i--
        ) {
      ...
      if (cameraRef.current) {
        ...
        cameraRef.current.position.x +=
          (cx - cameraRef.current.position.x) * 0.1 + rpmShake;
        cameraRef.current.position.z +=
          (cz - cameraRef.current.position.z) * 0.1 + rpmShake;
      ...
      if (buildingsRef.current && sidewalksRef.current) {
        ...
        if (
          !lastGridRef.current ||
          lastGridRef.current.x !== currentGridX ||
          lastGridRef.current.z !== currentGridZ
        ) {
          ...
          for (let i = -viewRadius; i <= viewRadius; i++) {
            for (let j = -viewRadius; j <= viewRadius; j++) {
              const bx =
                (currentGridX + i) * MAP.BLOCK_SIZE + MAP.BLOCK_SIZE * 0.5;
              const bz =
                (currentGridZ + j) * MAP.BLOCK_SIZE + MAP.BLOCK_SIZE * 0.5;
              const hash = Math.abs(
              ...
              // BUILDING
              const height =
                MAP.BUILDING_HEIGHT_MIN +
                (hash % 1) *
                  (MAP.BUILDING_HEIGHT_MAX - MAP.BUILDING_HEIGHT_MIN);
